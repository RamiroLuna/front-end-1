...
import { RptBonosService } from './rpt-bonos.service';

declare var $: any;
declare var Materialize: any;
@Component({
  selector: 'app-rpt-bonos',
  templateUrl: './rpt-bonos.component.html',
  providers: [RptBonosService]
})
export class RptBonosComponent implements OnInit {
  ...
  constructor(
    private service: RptBonosService,
  ...
  busqueda(parametrosBusqueda: any) {
    ...
    if (this.formConsultaPeriodo.valid) {
      ...
      this.service.getReporteBonos(this.auth.getIdUsuario(), parametrosBusqueda).subscribe(result => {

        if (result.response.sucessfull) {
          this.registros = result.data.bonos || [];
          this.registros = this.registros.filter(el=>el.padre==0);  
          this.titulo_tabla = " BONOS "+ this.getPeriodo(this.periodos, parametrosBusqueda.idPeriodo);
